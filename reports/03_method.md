...
> 停止・破棄)。旧oracle-spec値は「仕様追従能力の上界」の診断として保持。
...
**設計案**:
- 除外集合を正準化: eval側の全抽選(exact系のstdlib-500 ∪ P-J系のnp-500 ∪
  FICベアの5/feature)をJSON1本(runs/tables/eval_exclusion.json)に固定し、
  spec構築・同定系スクリプト全部がこれを読む(除外レシピ不整合48/500の
  恒久修正を兼ねる)。残り≈4,000ペア(feature当たり≈40)がspec構築プール。
...
- 語彙的に特異な編集(意味・語用の対事実)はfeature方向だけでは復元
  ...
  ことが予想され、それ自体が知見。oracle-spec値は「仕様追従能力の上界」
  として併記が自然。
