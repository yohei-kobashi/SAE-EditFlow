...
>
> **🏷️ 構成の名称整理(2026-07-24ユーザー指示 — 以後この名で統一)**:
> 要素名: **CTX-train**(旧T2: 学習キャッシュの文脈内z測定)/
> **CTX-spec**(旧⑦: 評価specの文脈内構築)/ **ADAPT**(旧T4v1:
> train区画適応)/ **BLEND(α)**(旧v3a: 重み補間)。
> 構成名: **Ours-ZS** = CTX-train+CTX-spec(zero-shot版、
> 0.142/0.140・床0.000/0.014)/ **Ours-AD** = Ours-ZSの重みを
> BLEND(0.3)へ置換(適応版、0.194/0.172・床0.002/0.030)。
> 旧内部名(T2/⑦/T4/blend)は04のログ内でのみ有効 — 論文・新規記述は
> 新名称を使う。k掃引・ablation等の分析はOurs-ZSで実施。
...
- **🏁 最終構成確定(07-24ユーザー決定)**: **主行=T2+⑦(zero-shot、
  ...
  適応行は**3層とも完了**(04§9u最終表): blend利得はL4 +35-43%/
  L12 +30-37%(E_enh 0.370=steer超え)/L20は床支配で無効。
  **測定キャンペーン完了 — 論文数値は§9u+§9t+§9r+§9sで確定**
