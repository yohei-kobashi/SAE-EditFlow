...
3. **介入本数 = 表出の局在性スペクトル**(新しい中心図):
   ...
   = 「3本では足りない」の実測。
   **🔴 推論の免許規則(2026-07-17、操作ハンドル≠feature対応)**:
   操作ハンドル(介入で編集が動く集合)には feature方向に加えて**事例の
   内容(WHAT)・整合性維持**が混入する(M0/P-B実測: rank17-32は方向と
   内容を分離不能に符号化)。したがって:
   - **書けない**: 「事例レベルkで編集成功 → featureの表現はk本」
     (生のk=32を表現幅と書かない)。書けるのは「因果的**操作**には
     top-3では足りない」= 操作インターフェース幅まで。
   - **書ける形(i)**: 現象レベル(target-free)r掃引 — 内容混入が構成上
     ない。P-J実測(r単調・true専有)は「**因果的に有効な現象表現は
     top-3より広い**」を支持。ただし「対応」の定義を検出的→因果的に
     取り替えたと明示(それ自体が本研究の主張の実演)。
   - **書ける形(ii)**: S_minの**事例横断の安定核**(Jaccard高い共通部分)
     = 因果検証済みの対応集合。事例ごとに変わる部分=内容、共通部分=
     現象、という操作的分解。安定核の本数・中身をFRC top-3と直接比較
     して初めて「対応するactivationsは3本より多い/別物」と書ける。
...
## 🟢 B-1 介入k掃引の実測(2026-07-17完走、499ペア、steer0.5、
runs/prod_gemma_v6/ksweep_steer/k*/report.md)

| k | true exact | random exact | net |
|---|---|---|---|
| 1 | 0.0681 | 0.0521 | +0.016 |
| 2 | 0.0802 | 0.0641 | +0.016 |
| 4 | 0.1082 | 0.0481 | +0.060 |
| 8 | 0.1503 | 0.0481 | +0.102 |
| 16 | 0.1944 | 0.0501 | +0.144 |
| 32 | **0.2365** | 0.0601 | +0.176 |
| 64 | 0.2385 | 0.0501 | +0.188 |

- **単調成長 → k=32で膝、32→64はほぼ飽和**(+0.002)。random床は全kで
  ~0.05-0.06に平坦(特異性維持)。
- **k=1はtargetを見た最良の1本ですら床**(0.0681 ≈ raw 0.0601、AxBench
  完全版0.0701とも同水準)— **単一latentの限界は選択の問題ではない**
  (P-J sanity gateと同じ結論を介入編集側で再証明。informedでも1本では
  編集は指令できない)。
- 免許規則どおりの読み: これは**操作インターフェース幅**(事例内容込み)
  の曲線 — 「最小対編集の因果的操作には~32本の介入で飽和」。M0(EF版、
