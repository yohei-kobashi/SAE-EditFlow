...
**下表はrng修正後の再判定値**(`run_frr_rerun.sh`、2026-07-15)。gemmaのみ
再判定待ちのためバグ影響下の値(要更新)。

| system | gemma-2-9b-it(旧・要再判定) | **GPT-4o(主)** | gpt-5.4-nano | exact |
|---|---|---|---|---|
| ef32 | (0.8169) | **0.8735** | **0.7679** | 0.2237 |
| routed | (0.7579) | 0.8306 | 0.7331 | **0.2839** |
| steer0.5 | (0.7609) | 0.7327 | 0.6922 | 0.2337 |
| steer net-FRR | (0.3192) | **0.4062** | 0.2221 | — |
| steer_rnd(偽陽性フロア) | — | 0.3265 | 0.4701 | — |

- **judge不変な主張(これを書く)**: **ef32が全judgeでFRR首位、routedが全
  judgeでexact首位** — 「FRRとexactはシステムを異なる順位で並べる」という
  多軸報告の必然性がjudge非依存に成立。正しい読みは「操舵再生成は方向づけ
  が上手いのではなく、当たった時に正確に着地する」。
- **有意性は自己一致率ではなく対応のある検定で判定**(§6e-3、
  `scripts/frr_paired_test.py`、同一ペア・同一judgeの厳密McNemar)。
  judgeノイズは980ペアの平均では打ち消し合い、差を**チャンス方向へ減衰
  させるだけで捏造しない**ので、観測された差は真の差の**保守的な下界**。
  実際、最も自己一致的なGPT-4oが最大の分離(ef32−steer **14.1pt**)を
  示し、最もノイジーなnanoが最小(7.6pt)= 減衰の署名。
...
**膨張量の実測(2026-07-15、仮定ではなく測定)** — FRR(旧)−FRR(新):

| system | GPT-4o | gpt-5.4-nano | 予測 |
|---|---|---|---|
| **routed** | **±0.0000**(0.8306→0.8306) | +0.0021 | **0**(元から独立順) |
| ef32 | +0.0030 | +0.0092 | >0 |
| steer | +0.0132 | +0.0266 | >0 |
| steer_rnd | +0.0144 | +0.0062 | >0 |

**routedがきっかり0.0000で、他の全システムが正に動いた** = 診断の因果構造
そのもの。個票は動いており(GPT-4o sys一致 0.9619)集計で相殺している。
gold方向一致は全て **1.0000** = goldキャッシュ再利用の検証も兼ねた。
**膨張は小さく(GPT-4o 0.3–1.4pt)結論は無傷**だが、nanoでは steer が
2.7pt 動いており、ノイジーなjudgeほど位置バイアスの影響も大きい。

### 6e-2. 自己一致率(rng修正後、3列とも正直な読み)

| judge | pooled自己一致 | flips | 非exact FRR(judgeが実仕事をする領域) |
|---|---|---|---|
| **GPT-4o(主)** | **0.9860 [0.974, 0.992]** | 10/714 | 0.7565 |
| gemma-2-9b-it | (0.9517 = 旧routed列のみ。再判定待ち) | 13/269 | 0.6849 |
| gpt-5.4-nano | 0.8789 [0.853, 0.901] | 86/710 | 0.6839 |

- **GPT-4oが最も自己一致的で、CIがnanoと明確に分離**(0.974–0.992 vs
  0.853–0.901)→ 主judge選定の実証的根拠。LinguaLens整合という外形的理由と、
  測定器としての品質が一致した。
- nanoは **gold順を再利用していた旧ef32/steer列でも19–21 flips** — 決定的
  ...
  **nanoを主judgeにできない技術的理由**として書ける。
- **減衰シグネチャ(強い傍証)**: judge信頼性がFRRの水準とシステム間分離を
  単調に予測する — GPT-4o(SC 0.9860)ef32−steer **14.1pt**、
  nano(0.8789)**7.6pt**。ノイズは差をチャンス方向へ**減衰**させるだけで
  捏造しない。最も静かな測定器が最大の差を見せる=**差は実在し、GPT-4oが
  最良の測定器**。
- judge間一致率(realized判定、n≈963–973、バグ影響下の旧値): routed
  gemma-GPT4o **0.8654** / GPT4o-nano 0.8193 / gemma-nano 0.7812 —
  **自己一致率上位2つ(GPT-4o・gemma)が互いに最も一致**し、nanoが外れる。
- 注意: 自己一致 ≠ 正確さ(一貫して間違うjudgeはあり得る)。主張は
  「GPT-4oが最良の測定器である傍証」までに留め、正確さは主張しない。
...
- この訂正で nano の扱いも変わる: nanoの小さいgap(7.6pt)は「床以下だから
  無意味」ではなく「ノイジーな測定器による減衰の結果」= 順位の一致は依然
  として証拠になる。
