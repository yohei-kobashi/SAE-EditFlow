...
- 「評価の幅」の中身: 顕著さjudge(LinguaLens)や生成rubric(AxBench)に
  対し、編集は (i) exactという厳密なテキスト接地基準、(ii) 介入本数の
  局在性スペクトル、(iii) FRR/net-FRRの特異性、(iv) 現象別の到達層
  (検出/WHERE/介入編集/条件付け)を同時に与える。
- **P-J/P-B/M0はこの枠組みの適用実例として書き直す**: 同じ編集基準が
...
3. **介入本数 = 表出の局在性スペクトル**(新しい中心図):
   ...
   少数×安定 = 真の辞書項目、少数×不安定 = 疎だが事例特異、多数 = 分散。
   測定器: (a) **介入k掃引**(steerの使用活性数 k=1..32 のexact曲線 —
   M0のEF版 0.060/0.126/0.210/0.190 の介入版が本命)、(b) **P-O介入版**
   (steer-exactペア上の最小介入集合 S_min、union/Jaccardで安定性)。
   既存資産の再ラベル: P-J r掃引 = 現象レベルの分散性、P-B(top-3で~7%)
   = 「3本では足りない」の実測。
4. **介入で編集できないfeature**: SAE側か手法側かの切り分けは困難 →
   **Limitation + 判別木で示唆**。現象ごとに
   検出(AUROC)/因果WHERE(P-I)/介入編集(steer・clamp)/条件付け編集(EF)/
   プロンプト(B2=SAE不使用参照)を突き合わせる:
   検出○・WHERE×・全効果器× → SAE側の示唆(交絡latent;
   universal_quantifiers の "proper names" latentが実例)。
   WHERE○・介入×・EF○ → 効果器側(情報は在るが介入で書けない=WHAT問題)。
...
**決定**: S1/S2/S3(hazard・特徴トークン・局所化)はどれもSAE活性の因果証明に
なっていない(条件付け=P(edit|Z=z)のまま)ため、**EF系を論文から完全に外す**。
コード・runs・台帳の記録は履歴として残すが、論文の主張・表・図に載せない。

**外れるもの**:
- S系列の全学習(S1/S2/S3、M1、S6)と手法節のEF記述(hazard分解・局所化・
  特徴トークン・真アラインメント)
- ef32 の全数値(exact 0.2237、FRR 0.8735 ほか)
- **routed = C0(0.2839/0.2892)** — count-ruleはEFのλ場に依存するため共倒れ
- C2(vs B2)・C3(vs パイプライン)— 編集器の主張なので消滅
- P-O の EF版(条件付け最小集合)、AxBench の ours_ef 腕
- λ-IoU の WHERE 表(EFのrate場の話)
...
- 判別木 — ただし**EF列を論文版から落とす**。C類(効果器側)の判定は
  「WHERE陽性 + B2可 + 介入不可」に弱化(「活性に情報がある」の直接証拠は
...
- **C0(ヘッドライン、2026-07-16用語訂正)**: SAE**条件付き**ハイブリッド
  ...
  ef32単独 0.2369。規則は完全教師なし・事前登録・凍結済み。
  **🔴 0.2839/0.2892 を「介入の結果」と呼んではならない** — headは条件付け
...
- **C1'(介入列のヘッドライン、2026-07-16実測確定)**: 介入の編集性能を
  ...
  |---|---|---|
  | clamp+再生成 | **0.0160**(LinguaLens完全版) | 0.1743(**11×**) |
  | steer+再生成 | **0.0701**(AxBench完全版)/ 0.0822(FRC×steer) | 0.2337 |

  **正しい床 = raw(無介入の書き換え役)0.0601**。これに対し:
  - 彼らの仕様の因果的寄与は **+0.006〜+0.022**(AxBench完全版 +0.010、
    random 0.0521 は raw より悪い)
  - **LinguaLens完全版(0.0160)は無介入を4倍下回る**。原因は再構成ダメージ:
    **recon対照 0.0100** — encode→decodeを通すだけで無情報書き換え役が
    0.060→0.010 に6倍崩壊。クランプ仕様の寄与は 0.010→0.016 の +0.006 のみ。
    「仕様の寄与 ≪ 機構の損傷」。
  - 我々の仕様は同じ機構で raw + 0.114(clamp)/ +0.174(steer)。
  - FRC×steer(0.0822)> AUROC×steer(0.0701)= P-Jの「FRCが特異的」と整合。
  - 注: pure仕様は抑制のみ(za空)なので clamp5/10/20/clampZ は**同一の介入**
    (クランプ値はamp側にしか使われない)— 4モード一致は構成上の必然でバグ
    ではない。現象specでは steer1 > steer0.5(抑制のみのdvecは弱く、強いαが
    要る — 事例specの崖と逆)。
  - 対応検定は records から取得可能(要実行、執筆時)。
  **主張文**: 「介入機構を固定したまま、仕様を各論文のプロトコル(FRC top-3 /
  単一AUROC latent)から我々の事例レベル編集局所deltaに替えると、exactは
  無介入床+0.01〜0.02 から 0.17〜0.23 へ上がる。介入の編集力は仕様が決める。」
  編集器を一切含まない介入のみの主張。promptingに負けるのは許容済み。
...
- **✅ sanity gate 通過(2026-07-16実測)**: top-1 AUROC は
  **mean 0.939 / median 0.979 / 74/99現象で≥0.9** — AxBench自身のSAE-A平均
  (0.917)を上回る。**選択器は機械的にはほぼ完璧に機能している**。
  → auroc_r1 のゼロ信号は選択失敗ではなく**単一latentの限界**と確定。
  逸話的傍証: universal_quantifiers の AUROC=1.000 latent のNeuronpedia
  ラベルは「proper names and titles」— **識別的に完璧な特徴が現象と無関係な
  交絡でありうる**ことの実例(逸話として1文、主証拠は数値)。
- **対応検定(delta1、500ペア、2026-07-16)**:

  | セル | true>rnd / true<rnd | p | 読み |
  |---|---|---|---|
  | auroc_r1(AxBench式) | 27 / 11 | 1.4e-02 | **平均比 0.86×(trueが下)**。整合した因果信号なし |
  | frc_r3(LinguaLens式) | **67 / 0** | **1.4e-20** | 微弱(0.17発火)だが**完全にtrue専有** |
  | frc_r32 | 277 / 3 | 3.8e-78 | 特異性27× |
  | auroc_r32 | 364 / 29 | 7.3e-75 | 強いが漏れる(3.9×) |

  **論文の1文**: 「AUROC選択器は検出をほぼ完璧にこなす(mean 0.939)が、
  その単一latentは因果的に何も動かさない(平均比0.86×)。一方FRCの3本は
  微弱でも純粋な因果信号を持つ(67/0、p=1e-20)— **検出の完璧さと因果的
  実在は、同一パイプライン内で乖離する**。」
...
## 6l. P-O — 事例レベル最小条件集合(SAE活性の絞り込み、2026-07-17設計・実装済み)

**ユーザ要望**: SAE本来の目的 = featureに対応するactivationの特定。minimal pair
1つに対応する単一activationは無い(組合せ)と分かったが、**できるだけ絞り込む**
オプション機能が欲しい。

**設計(`scripts/prune_spec.py`)**: M0が棄却したのは**大域的**絞り込み
(k32→k8で全ペア一律)。P-Oは**事例ごと**に絞る — editorがexactで解いた
ペア上で、k=32仕様を「exactを保つ最小部分集合 S_min」まで刈り込む:
1. |delta|降順プレフィクスの**二分探索**(~5デコード、端点は検証)
2. 生存特徴の**後退消去**(小さい順、~p*デコード)+ 最終検証
デコードはチャンピオン構成そのまま(thr0.1・48steps・lens λ=1)。
コスト ~15-20デコード/ペア、60ペアで1セッション内。resume対応。

**検定する仮説(=現象レベル失敗族の機構的説明の完成)**:
「**|S_min|は事例ごとには小さいが、中身が事例ごとに違う**」
→ report が per-phenomenon で mean|S_min| / **union** / pairwise Jaccard /
最頻特徴(Neuronpediaラベル付き)を出す。union ≫ |S_min| かつ低Jaccard =
「事例あたり数本で足りるが、現象を通す固定の数本は存在しない」— P-B崩壊・
P-J r=3微弱・AxBench r=1ゼロの説明が1枚で立つ。逆に高Jaccardの現象は
**因果検証済みの安定コア**を持ち、FRC/AUROCより良い辞書の種になる。

**実行順**: S6決着後に、その時点のチャンピオンで実行(S6がGOならS6で、
NO-GOならS3で)— 公開するシステムの分析であるべきなので。
検証済み: 署名照合(load_editflow_from_checkpoint の引数順を修正)、
絞り込みロジックは偽デコーダ上で真の最小集合を6デコードで回収。
...
4. 指標間の順位反転も記録: LinguaLens指標では clamp(+0.631)> steer
   ...
   主題の追加実例。
5. **raw(無介入)参照が確定(2026-07-17補完)**: P(absent|raw) = **0.303**、
   AxBench score(raw)= **1.081** [concept 1.05 / instruct 1.93 / fluency 2.00]。
   これで両プロトコルの絶対位置が確定:
   - **LinguaLens完全版はP(absent) 0.220 < raw 0.303 — 無介入より特徴を
     消せない**(copy 46%が原因: 彼らのプロトコルは無情報rewriterより
     編集しない)。AxBench指標でも 0.570 < raw 1.081 で**無介入未満**。
   - **AxBench完全版は raw とほぼ同一**(P(absent) +0.046、AxBench score
     +0.04)— 彼らのプロトコルは彼らの指標上で無介入と区別できない。
   - 我々の仕様は同じ機構で raw を明確に超える: P(absent) 0.613/0.693
     (+0.31/+0.39)、AxBench 1.211/1.262(concept +0.27/+0.28)。
   **C1'の最終形(彼らの通貨で)**: 「両論文のプロトコル完全版は、両論文
   自身の指標において、無情報の書き換え役と同等以下。同じ機構に我々の
   事例レベル仕様を与えると、全指標で明確に超える。」
...
- **🔴 S6判定(2026-07-17、probe500完走)= NO-GO、S3チャンピオン続投**:
  - (ii) exact **0.0461**@thr0.1 / 0.0741@thr0.5 — S3の0.2010から**4×崩壊**
    (ゲート≥0.2104に遠い)。sim 0.29/0.36、**copy 0.03-0.09 = 激しい
    過剰編集**。
  - (iii) empty no_edit **1.0000 ✓** だが random no_edit **0.5591**@thr0.1
    (S3 0.87、P5の狙いと逆方向に悪化)。thr0.5では0.9038。
  - (iv) λ-IoU 0.7305(ゲート0.74に僅差負け)、randomランキング0.3515。
  - (i) MOVE: dev q_mov **0.5000 = pointerがチャンス** — 学習されず
    (SPLITINF 17件では予想通り)。
  - 較正も劣化: on-dist ratio t=0.9で0.326(S2/S3のt安定~0.5が崩れた)。
  - **読み**: P5(mismatched-z null)は「一致しないzでは黙れ」を教える
    はずが、この構成(v7語彙多様化 + move-ops + null 0.12 同時)では
    逆に発火の抑制が壊れた。交絡が3つ同時なので帰属不能 — M1の規律
    (診断なき単独再挑戦はしない)に従い、**S6系は凍結**。ours_ef腕・
    P-O条件付け版はS3で実施(事前登録どおり)。
  - 単離ablation(v5キャッシュ+P5のみ等)は任意の将来項目。

## 6i. S5(family仕様学習)への懸念と、B1改善の実行順(2026-07-16)

**🔴 S5のfamily集合案はユーザ指摘により要修正**: corruption familyは25個
しかなく、**特徴ID集合が25パターンに固定される** → 条件付けencoderが
「25個の集合埋め込み」を暗記でき、99の未知現象集合への汎化を測る土俵が
崩れる。仕様の多様性が事例レベル(ペアごとに異なる)から大きく退化する。
- **多様性を保つ代替(設計メモ、未実装)**: 現象ラベル不要の
  **擬似現象クラスタリング** — corruptionの事例deltaの疎サポートを
  K-meansでK個(K=500〜数千、25→∞のダイヤル)に束ね、クラスタ平均集合を
  擬似現象仕様として学習に混ぜる。集約仕様の「型」は学べて、集合の暗記は
  Kで防ぐ。S5を実行する場合はこの形。
- ただし優先度は低下: P-B対照(FRC中身は~7%)とP-I(WHAT不能)から、
  現象レベル仕様の天井自体が低い可能性。執筆と並行可能な場合のみ。

**B1改善の実行順(カテゴリ不要・学習不要)**:
1. **P-L(`run_b3_local.sh`)**: 局在化を**再生成**側へ — prefillは抑制特徴
   発火位置のみ介入(読取を守る)、生成トークンは常時介入(書く方は操舵)。
   readoutで検証済みのマスクを、効く効果器に接続する未測定セル。
   併せてαの細目掃引(0.25/0.375/0.5/0.75; 0.5→1で半減する崖の頂点確認)。
   **✅ P-L判定(2026-07-17実測)= null、診断付き。** trueの3/4のαで
   local ≡ all が4桁一致(0.0982/0.1764/0.2385)、random行は明確に異なる
   (copy 0.1383 vs 0.1683)→ **マスクは適用されているが、trueの抑制特徴
   32本はプロンプトほぼ全域で発火し、マスクが実質全域** = 局在化の余地なし
   (P-I v2の「trueでマスクほぼ全域」兆候と整合)。null につき反例(e)
   サーベイ不要、ablation脚注扱い。
   **αの頂点確定**: 0.25→0.375→0.5→0.75 = 0.098→0.176→**0.2385**→0.18。
   0.5が鋭い頂点、off-gridに伸び代なし。**介入列の最終値 = steer0.5
   (0.2385@499 / 0.2337@997)で確定、C1'の右列もこのまま**。
   副産物: バケット別最適αが編集規模で分かれる(1-opは0.75で0.1717、
   2-3/4-8は0.5)。バケット別オラクルα ≈ 0.263 — ただし規模推定器が要る
   (λ場カウント等)ため任意の将来項目(P-M候補、未実施)。
