...
## R5. 第5ラウンド(2026-07-17)— 🔵再固定が生んだ新主張への反例狩り
(**深掘り済み: 9本全てY-1手順で自前PDF抽出・逐語検証完了 2026-07-17**)

**対象**: (e)「編集可能性によるSAE活性の因果評価枠組み」、(f)「介入本数=
局在性スペクトル」。**判定: 直接の反例なし(9/9確認)**。逐語検証で確定した
決定的事実:
- CausalGym全文に **"SAE"/"sparse autoencoder" は0回**。成功基準は
  log odds-ratio(式9)= 次トークン挙動。逐語: "The core idea of
  intervention is adopted directly from the do-operator used in causal
  inference; we test the intervention's effect on model output to
  establish a causal relationship."
- SAEBench全文に **"minimal pair" 0回・"text edit" 0回**。逐語: "a
  comprehensive evaluation suite that measures SAE performance across
  eight diverse metrics, spanning interpretability, feature
  disentanglement and practical applications like unlearning."
- Hase et al. 逐語: "localization conclusions from representation
  denoising (also known as Causal Tracing) do not provide any insight
  into which model MLP layer would be best to edit" / "which layer we
  edit is a far better predictor of performance."
- Engels et al. 逐語: "a rigorous definition of irreducible
  multi-dimensional features based on whether they can be decomposed
  into either independent or non-co-occurring lower-dimensional
  features"(円環の曜日・月の実例)。
- SFC 逐語: "causally implicated subnetworks of human-interpretable
  features for explaining language model behaviors"。SHIFTは分類器の
  脱バイアス("surgically removing sensitivity to unintended signals")
  でありテキスト編集ではない。"minimal pair" 0回。

**🆕 未読候補4本の判定(全てPDF確認済み)**:
1. **arXiv:2510.01246**(SAE vs activation-diff steering)— **(f)の対抗
   仮説として引用必須**: "many dimensions among the top-k latents capture
   nonsemantic features such as punctuation ... we propose focusing on a
   single, most relevant SAE latent (top-1), eliminating redundant
   features"。**top-1で足りる(むしろ良い)と主張** — ただし基準は
   命令追従のsteering品質で、minimal pair 0回・exact基準なし。我々の
   k掃引が「編集には多数要る」を出せば、**評価器(生成品質 vs 厳密編集)
   が局在性の結論を変える**という書き方で両立させる(反例ではなく緊張)。
2. **Beyond Input Activations**(EMNLP 2025 main.87)— **P-Jの書き方を
   制約**: "activated latents do not contribute equally ... only latents
   with high causal influence are effective for model steering" —
   **「入力側活性ではなく出力への因果影響でlatentを選ぶべき」は先行**。
   P-Jの新規性を「発想」に置かず、**両論文の公式選択法(FRC/AUROC)を
   編集・readoutの因果基準で測った初の対決**+検出sanity gate付き、に置く。
3. arXiv:2507.08473(説明なしSAE評価)— 活性予測ベースの評価改革。
   編集なし・minimal pairなし。SAE評価ランドスケープの脚注のみ。
4. EACL 2026 SRW(SAE言語steeringの品質影響)— 言語切替steeringの
   タスク性能影響。ランドスケープのみ、脅威なし。

以下の表は要約ベースの初回パス(逐語は上記で検証済みに更新):

| 候補 | 内容(要約ベース) | 我々との差分(書き方) |
|---|---|---|
| **CausalGym**(Arora, Jurafsky, Potts — ACL 2024, arXiv:2402.12560)| SyntaxGym由来の**言語学的minimal pairs**で介入手法(DAS/probe/means等)を因果ベンチマーク。do演算子の枠組み | **最近接**。彼らの成功基準は**モデルの次トークン挙動のフリップ**、我々は**出力テキストの厳密なminimal-pair編集**。彼らの手法集合に**SAE辞書latentは無い**(教師あり特徴化が中心)。「挙動フリップ→テキスト編集」への一般化+SAE活性への適用が差分 |
| **RAVEL / SAEBench**(arXiv:2503.09532 ほか)| SAEの**介入ベース評価**は既存: RAVEL(latent介入で属性予測を変え他属性を保存)、SAEBench(包括ベンチ、RAVEL・absorption等収録)| 「SAEを介入で評価する」こと自体は先行あり — **新規性をそこに置かない**。差分は評価器: 属性予測のフリップ vs **自由テキストのminimal-pair編集**(exact+文法性+局在性スペクトル)。「編集という評価器が検出完璧なlatentの因果不活性(P-J)を暴く」が我々の追加 |
| **Sparse Feature Circuits**(Marks et al., ICLR 2025, arXiv:2403.19647)| SAE特徴の**因果帰属**(挙動への寄与を線形近似で同定・編集)| SAE特徴の因果性研究の主要系譜として引く。回路帰属(挙動)vs 編集実行(テキスト)。SHIFT等の「編集」は分類器のバイアス除去で、テキスト編集ではない |
| **Hase et al.**(NeurIPS 2023, arXiv:2301.04213)| **localization ≠ editing**: causal tracingの局在化が重み編集の最適位置を予測しない | 我々の「検出≠指令」と同型の乖離を**重み編集ドメイン**で示した先行。3層分解の系譜として引く(D.1のamnesic probingと並置) |
| **Engels et al.**(ICLR 2025, arXiv:2405.14860)| 多次元(円環)特徴の存在 — 1概念=1latentの反例 | **(f)局在性スペクトルの動機**として引く: featureが複数活性に分散しうることは既知。我々の差分は**編集可能性を基準にした分散度の測定**(介入k掃引・S_min) |
| 未読候補(要トリアージ)| arXiv:2507.08473(説明なしSAE評価・介入予測)、arXiv:2510.01246(SAE vs activation-diff steering比較)、EACL 2026 SRW「SAE-based Language Steering」、EMNLP 2025「Beyond Input Activations」 | (e)への潜在的近接 — 深掘りパスで各PDFを確認 |

**(e)(f)の現時点の判定**: 「minimal-pairの**テキスト編集**を成功基準として
SAE活性の因果妥当性を評価する」構成の直接の先行例は未発見。ただし
「介入ベースのSAE評価」(SAEBench/RAVEL)と「minimal pairsでの因果
ベンチマーク」(CausalGym)が両側から近接しており、**貢献文は両者の合成として
書く**: 「CausalGymの言語学的因果評価をテキスト編集の実行まで拡張し、
SAEBench系の介入評価に編集という新しい評価器を加える」。
**普遍否定を書く前に深掘りパス(PDF逐語)必須**。
